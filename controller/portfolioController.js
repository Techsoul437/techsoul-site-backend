...
export const createPortfolio = async (req, res) => {
  const { banner, logo, asset, section, title, duration, type, description } =
    req.body;
...
export const getAllPortfolios = async (req, res) => {
  try {
    ...
    const portfolios = await Portfolio.find(filter).limit(limit);
...
export const updatePortfolio = async (req, res) => {
  ...
  try {
    ...
    if (title && title !== portfolio.title) {
      const isExist = await Portfolio.findOne({
        title,
